...
/**
 * Описание главной — для `<meta name="description">` и узла `WebPage` в
 * JSON-LD. Одна строка на два места: пока их было две, правка описания
 * молча оставляла разметку со старым текстом.
 *
...
/** Заголовок и описание /specialists — по той же причине в одном месте. */
export const SPECIALISTS_TITLE =
  "Базилик для специалистов — планы питания вместо Word и PDF";
export const SPECIALISTS_DESCRIPTION =
...
/**
 * Адрес карточки ссылки, которую собирает app/opengraph-image.tsx. Next
 * подписывает его в `og:image` хешем в query, но сам маршрут отвечает и
 * без него — на него и ссылается `primaryImageOfPage` в JSON-LD.
 */
